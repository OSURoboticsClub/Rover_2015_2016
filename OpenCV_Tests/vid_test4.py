...
while(cv2.waitKey(1) & 0xFF != ord('q')): #Quit window using the 'q' key 
    ...
    disp = cv2.bilateralFilter(disp,9,200,200) #blur image for use    

    # Attempt Contours on disparity    
    # ret, thresh = cv2.threshold(disp, 127,255,0)
    # contoursD, hierarchy = cv2.findContours(thresh, 1,2)    
    # cv2.drawContours(disp, contoursD, -1, (50,255,50), 3) #draw Right
    ...
    cv2.drawContours(imgL, contoursL, -1, (50,255,50), 3) #draw Left
    ret, thresh = cv2.threshold(imgR, 127,255,0)
    contoursR, hierarchy = cv2.findContours(thresh, 1,2)    
    cv2.drawContours(imgR, contoursR, -1, (50,255,50), 3) #draw Right
